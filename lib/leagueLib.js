require("dotenv").process.env({silent: true}); //keep api key in .env
    ...
    function(error, response, body) {
        if (!error && response.statusCode === 200) { //GAME FOUND
            ...
            if (SEArg.getInit() === false) {
                ...
                SEArg.setInitial(body.gameMode, body.gameType, body.gameStartTime, body.gameQueueprocess.envId, body.mapId);
